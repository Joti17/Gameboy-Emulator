...
void SquareChannel::clockEnvelope()
{
    ...
    {
        envTimer = envPeriod;
        if (envAdd && envVolume < 15)
            ++envVolume;
        else if (!envAdd && envVolume > 0)
            --envVolume;
    }
}
...
void Sound::write8(uint16 addr, uint8 val)
{
    ...
    case 0xFF25:
        NR51 = val;
        sq1.rightOut = val & 0x01;
        sq1.leftOut = val & 0x10;
        sq2.rightOut = val & 0x02;
        sq2.leftOut = val & 0x20;
        wave.rightOut = val & 0x04;
        wave.leftOut = val & 0x40;
        noise.rightOut = val & 0x08;
        noise.leftOut = val & 0x80;
